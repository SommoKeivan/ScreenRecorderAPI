...
public:
	...
	static Expected<Ptr<SimpleInputFormat>> create_desktop(bool enableAudio = false) noexcept
	{
		AVFormatContext* ic  = avformat_alloc_context();
		AVInputFormat* av_if = av_find_input_format("dshow"); //gdigrab -> solo video; dshow -> video e audio
		AVDictionary* opts = nullptr;
		//TODO
		av_dict_set(&opts, "rtbufsize", "1024M", 0);
		/* av_dict_set(&opts, "sample_fmt", "fltp", 0);*/
		av_dict_set(&opts, "framerate", "15", 0);
		auto err = avformat_open_input(&ic, "video=screen-capture-recorder:audio=virtual-audio-capturer", av_if, &opts);//gdigrab -> desktop;
									   																				    //dshow -> video=UScreenCapture o video=screen-capture-recorder
		if (err < 0)
			RETURN_AV_ERROR("Cannot open input 'desktop': {}", avErrorStr(err));
