...
public:
	static Expected<Ptr<StreamReader>> create(std::string_view url, bool enableAudio = false) noexcept
	{
		...
		Expected<Ptr<SimpleInputFormat>> iformExp = SimpleInputFormat::create(url, enableAudio);
	...
	static Expected<Ptr<StreamReader>> create_desktop(bool enableAudio = false) noexcept
	{
		Ptr<StreamReader> sr{new StreamReader};
		Expected<Ptr<SimpleInputFormat>> iformExp = SimpleInputFormat::create_desktop(enableAudio);
	...
	auto codecID() const noexcept
	{
		return std::get<1>(aStream_)->native()->codec_id;
	}

	auto bitRate() const noexcept 
	{
		return std::get<1>(aStream_)->native()->bit_rate;
	}
